=head1 NAME
    ...
    Lingua::DE::Fathom - Measure readability of German text

=head1 SYNOPSIS

    use Lingua::DE::Fathom;

    my $text = Lingua::DE::Fathom->new();

    $text->analyse_file("sample.txt"); # Analyse contents of a text file

    $accumulate = 1;
    my $text_string = q{
    Returns the number of words in the analyzed text file or block. A word must
    ...
    Common abbreviations such as U.S. or numbers like 1.23 will not denote the end of
    a sentence.
    };
    ...
    # Methods to return statistics on the analyzed text
    $text->num_chars;
    $text->num_words;
...
Lingua::EN::Syllable, Lingua::DE::Sentence
...
The C<new> method creates an instance of a text object This must be called
...
argument may be supplied to control the accumulation of statistics. If set to
...
Returns the number of characters in the analyzed text file or block. This
...
Returns the percentage of complex words in the analyzed text file or block. A
complex word must consist of three or more syllables. This statistic is used to
calculate the fog index.

=head2 num_sentences

Returns the number of sentences in the analyzed text file or block. A sentence
...
Returns the number of lines containing some text in the analyzed
text file or block.

=head2 num_non_text_lines

Returns the number of lines containing no text in the analyzed
text file or block.

=head2 num_blank_lines

Returns the number of empty lines in the analyzed
text file or block.

=head2 num_paragraphs

Returns the number of paragraphs in the analyzed text file or block.

=head2 syllables_per_word

Returns the average number of syllables per word in the analyzed
text file or block.
...
well-formed and logical. You could analyze a passage of nonsensical English and
...
Returns the Fog index for the analyzed text file or block.
...
Returns the Flesch reading ease score for the analyzed text file or block.
...
This score rates text on the U.S. grade school level. So a score of 8.0 means
that an eighth grader can understand the document. A score of 7.0 to
...
Lingua::DE::Fathom was written by Kim Ryan <kimryan at cpan dot org>.

=head1 COPYRIGHT AND LICENSE

Copyright (c) 2024 Helmut Wollmersdorfer. All rights reserved.
...
our $hyphenator = Text::Hyphen::DE->new;
...
sub analyse_file {
   ...
   # Only analyze non-empty text files
...
sub report {
   ...
   #my $num_unique_words = scalar keys %{$text->unique_words};
   my $num_unique_words = scalar keys %{ $text->{unique_words} };
   my $type_token_ratio = $num_unique_words / $text->num_words;
   ...
   $report .= sprintf("Number unique words        : %d\n",  $num_unique_words);
   $report .= sprintf("Number simple words        : %d\n",  $text->{num_simple_words});
   $report .= sprintf("Number long words          : %d\n",  $text->{num_long_words});
   $report .= sprintf("Percent of complex words   : %.2f\n",$text->percent_complex_words);
   $report .= sprintf("Type-Token-Ratio (TTR)     : %.2f\n",$type_token_ratio);
...
sub _initialize {
   ...
   $text->{num_simple_words} = 0;
   $text->{num_long_words} = 0;
...
#------------------------------------------------------------------------------
# Try to detect real words in a line. Increment syllable, word, and complex word counters.
...
sub _analyse_words {
   ...
   my @words = $one_line =~ /([a-zäöü]['a-zäöü-]*)/ig;

   print '@words: ',join(', ',@words),"\n";

   #while ( $one_line =~ /\b([a-zäöü]['a-zäöü-]*)\b/ig ) {
   #  my $one_word = $1;
   for my $one_word (@words) {

      # Try to filter out acronyms and  abbreviations by accepting
      # words with a vowel sound. This won't work for GPO etc.
      next unless $one_word =~ /[aeiouyäöü]/i;

      # Test for valid hyphenated word like be-bop
      if ( $one_word =~ /-/ ) {
         next unless $one_word =~ /[a-zäöü]{2,}-[a-zäöü]{2,}/i;
      }
      ...
      my $num_syllables_current_word = scalar(split(/-+/,$hyphenator->hyphenate($one_word, '-')));
      print $hyphenator->hyphenate($one_word, '-'),
        ' ', $num_syllables_current_word,' ', length($one_word),"\n";
      $text->{num_syllables} += $num_syllables_current_word;
      ...
      elsif ( $num_syllables_current_word == 1 ) {
         $text->{num_simple_words}++;
      }

      # long_words >= 6 chars
      if ( length($one_word) > 6 ) {
         $text->{num_long_words}++;
         #print 'long word: ',$one_word,"\n";
      }
   }
...
sub _calculate_readability {
   ...
   if ( $text->{num_sentences} and $text->{num_words} ) {
      ...
      $text->{vienna} = 0.1935 * ( $text->{num_complex_words} / $text->{num_words} )
        + 0.1672 * $text->{words_per_sentence}
        + 0.1297 * ( $text->{num_long_words} / $text->{num_words} ) * 100
        - 0.0327 * ( $text->{num_simple_words} / $text->{num_words} ) * 100
        - 0.875;
   }
   else {
      $text->{words_per_sentence} = 0;
